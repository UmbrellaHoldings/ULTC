...
CPubKey CWallet::GenerateNewKey()
{
  ...
  // default to compressed public keys if we want compcoin 0.6.0 wallets

  RandAddSeedPerfmon();
  CKey secret;
  secret.MakeNewKey(fCompressed);

  // Compressed public keys were introduced in version compcoin 0.6.0
  if (fCompressed)
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

  {
    ...
    // Encryption was introduced in compcoin version 0.4.0
  ...
  CKeyPool keypool;
  {
    ...
    if (nIndex == -1)
    {
      if (fAllowReuse && vchDefaultKey.IsValid())
      {
        ...
        LOG() << "(default key) " << result << std::endl;
        return true;
      }
      if (IsLocked()) return false;
      result = GenerateNewKey();
      LOG() << "(-1) " << result << std::endl;
      return true;
    }
    KeepKey(nIndex);
    result = keypool.vchPubKey;
  }
  LOG() << "(ok) " << result << std::endl;
  return true;
}
