...
{
    vector<unsigned char> vchAuxWithHeader(
      UBEGIN(pchMergedMiningHeader), 
      UEND(pchMergedMiningHeader)
    );
    vchAuxWithHeader.insert(
      vchAuxWithHeader.end(), 
      vchAux.begin(), 
      vchAux.end()
    );

    // Push OP_2 just in case we want versioning later
    return CScript() << nBits << nExtraNonce << OP_2 
      << vchAuxWithHeader;
}
...
void IncrementExtraNonceWithAux(CBlock* pblock, CBlockIndex* pindexPrev, unsigned int& nExtraNonce, vector<unsigned char>& vchAux)
{
    ...
    pblock->vtx[0].vin[0].scriptSig = MakeCoinbaseWithAux(
      pblock->nBits, 
      nExtraNonce, 
      vchAux
    );
